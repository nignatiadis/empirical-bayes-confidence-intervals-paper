...
function evaluate_at_n(true_G, amari, floc, target, n; ci_only=false)
    ...
    if ci_only
        floc_fit = nothing
        amari_fit = nothing
        floc_ci = confint(floc, target, discr.(Zs))
        amari_ci = confint(amari, target, Zs)

    else
        floc_fit = fit(floc, target, discr.(Zs))
        floc_ci = confint(floc_fit)
        amari_fit = fit(amari, target, Zs)
...
results_density = Vector{Any}(undef, length(ns))

for (i,n) in enumerate(ns)
    results_density[i] = evaluate_at_n(true_G, amari_dkw,
                        floc_method, density_target, n; ci_only=false)
end


α = 0.05
amari_ci_lengths = [res.amari_ci.upper - res.amari_ci.lower for res in results_density]
dkw_ci_lengths = [res.floc_ci.upper - res.floc_ci.lower for res in results_density]
...
zs = 0:7
Qs_3 = results_density[3].amari_fit.Q.(PoissonSample.(zs))
Qs_7 = results_density[7].amari_fit.Q.(PoissonSample.(zs))
Qs_11 = results_density[11].amari_fit.Q.(PoissonSample.(zs))
...
plot!(Q_plot, zs .+ 0.15, Qs_11, seriestype=:sticks,  label = L"n=10^9",
...
results_postmean = Vector{Any}(undef, length(ns))

Random.seed!(1)

for (i,n) in enumerate(ns)
    results_postmean[i] = evaluate_at_n(true_G, amari_dkw,
                        floc_method, postmean_target, n; ci_only=true)
end


α = 0.05
postmean_amari_ci_lengths = [res.amari_ci.upper - res.amari_ci.lower for res in results_postmean]
postmean_dkw_ci_lengths = [res.floc_ci.upper - res.floc_ci.lower for res in results_postmean]
var_delta = (K+1)^2*f4/abs2(f3)*(1 + f4/f3)
